...
export function WorkplanDetailShell({
  ...
  const remainingCount = workplanSections.length - completedCount;
    ...
    <div className="grid gap-6">
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-ink/60">
                Werk het plan stap voor stap af en gebruik de tabs om snel te zien waar nog informatie mist.
              </p>
            </div>
        ...
        <Panel className="bg-forest text-white">
          <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-white/45">Status</p>
          <div className="mt-4 grid gap-3">
            <div className="rounded-[20px] bg-white/8 px-4 py-4">
              <p className="text-sm text-white/70">Actieve stap</p>
              <p className="mt-1 text-lg font-semibold">{current.title}</p>
            </div>
            <div className="rounded-[20px] bg-white/8 px-4 py-4">
              <p className="text-sm text-white/70">Nog te doen</p>
              <p className="mt-1 text-lg font-semibold">{remainingCount} sectie(s)</p>
            </div>
          </div>
        </Panel>
      </div>
          ...
          {workplanSections.map((section, index) => {
            ...
            return (
              <button
                ...
                className={`flex min-w-[176px] items-center justify-between gap-3 border-r border-line px-4 py-4 text-left transition ${
      ...
      <Panel>
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="rounded-[18px] border border-line bg-mist/60 px-4 py-3 text-sm text-ink/65">
            {activeIndex + 1} / {workplanSections.length}
          </div>
        </div>
        <div className="mt-5">
          <WorkplanSectionForm
